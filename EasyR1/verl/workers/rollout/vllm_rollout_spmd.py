...
def _process_multi_modal_data(
    ...
    if "videos" in multi_modal_data:
        video_metadata_list = multi_modal_data.get("video_metadata")
        for idx, video in enumerate(multi_modal_data["videos"]):
            processed_video, sample_fps, metadata = process_video(
                video,
            ...
            if video_metadata_list is not None and idx < len(video_metadata_list):
                metadata = video_metadata_list[idx]

            if metadata is None:
                inferred_fps = float(sample_fps) if sample_fps is not None else float(video_fps)
                num_frames = int(processed_video.shape[0]) if hasattr(processed_video, "shape") else 0
                metadata = {
                    "fps": inferred_fps,
                ...
                metadata.setdefault("do_sample_frames", False)

            videos.append((processed_video, metadata))

        # use presampled frames produced by qwen_vl_utils
        if len(videos) > 0:
