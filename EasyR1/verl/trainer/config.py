...
class DataConfig:
    ...
    video_dir_map: dict[str, str] = field(default_factory=dict)
    video_fps: float = 2.0
    ...
    def post_init(self):
        ...
        if self.video_dir_map:
            resolved_video_dir_map = {}
            for source, path in self.video_dir_map.items():
                if os.path.exists(path):
                    resolved_video_dir_map[source] = os.path.abspath(path)
                else:
                    print(f"Video directory for data source `{source}` not found: {path}")
                    resolved_video_dir_map[source] = path
            self.video_dir_map = resolved_video_dir_map
